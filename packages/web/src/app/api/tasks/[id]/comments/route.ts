...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!workspace || !actor || !text) {
      return fail('tasks.comment', 'VALIDATION_ERROR', 'workspace, actor, and text are required');
    }

    await taskService.addComment(taskId, workspace, text, actor);
    return ok('tasks.comment', { task_id: taskId, actor });
  } catch (err) {
    if (err instanceof TaskForgeError) {
      const status = err.code === 'TASK_NOT_FOUND' ? 404 : 400;
      return fail('tasks.comment', err.code, err.message, status);
    }
    return fail('tasks.comment', 'UNKNOWN_ERROR', String(err), 500);
  }
}
